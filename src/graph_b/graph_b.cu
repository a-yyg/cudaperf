...
// Add all intermediate matrices together two by two asynchronously
// example: C1 = B1 + B2, C2 = B3 + B4, ..., C(N/2) = B(N-1) + BN
// then, repeat the process with the new intermediate matrices
// example: C(N/2 + 1) = C1 + C2, C(N/2 + 2) = C3 + C4, ...
// repeat until we have only one matrix left. The result will be saved in CN
// We will only consider the case where N is a power of 2 for simplicity
// in this case, the final result will be stored in C(N - 1)
class GraphB {
public:
  GraphB(int N, int numScalars) : m_N(N), m_numScalars(numScalars) {
  ...
  void exec(int iterations = 1) {
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      int offset = 0;
      int current_num = m_numScalars;

      // Add all intermediate matrices together two by two asynchronously
      // First B1 + B2 = C1, B3 + B4 = C2, ...
      for (int i = 0; i < current_num; i += 2) {
        MatAdd<<<dimGrid, dimBlock, 0, streams[i / 2].get()>>>(
            m_C[i / 2].get(), m_B[i].get(), m_B[i + 1].get(), m_N);
      }

      current_num /= 2;
      offset += current_num;

      // then C1 + C2 = C(N/2 + 1), C3 + C4 = C(N/2 + 2), ...
      while (current_num > 2) {
        for (int i = 0; i < current_num; i += 2) {
          // printf("offset: %d, current_num: %d, i: %d\n", offset, current_num, i);
          // printf("C[%d] = C[%d] + C[%d]\n", i / 2 + offset, i + offset - current_num, i + 1 + offset - current_num);
          MatAdd<<<dimGrid, dimBlock, 0, streams[i / 2].get()>>>(
              m_C[i / 2 + offset].get(), m_C[i + offset - current_num].get(),
              m_C[i + 1 + offset - current_num].get(), m_N);
        }
        current_num /= 2;
        offset += current_num;
      }

      // lastly, C(N - 3) + C(N - 2) = C(N - 1)
      MatAdd<<<dimGrid, dimBlock, 0, streams[0].get()>>>(
          m_C[m_numScalars - 2].get(), m_C[m_numScalars - 3].get(),
          m_C[m_numScalars - 4].get(), m_N);
...
int main(int argc, char *argv[]) {
  ...
  // Create the graph
  YuriPerf::GraphB graph(N, numScalars);
