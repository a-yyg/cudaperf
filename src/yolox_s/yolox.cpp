...
static void decode_outputs(float* prob, std::vector<Object>& objects, float scale, const int img_w, const int img_h) {
    ...
    // {
    //     char temp[256];
    //     snprintf(temp, 256, "%d %d %d %d %d %f\n", objects[i].label, (int)objects[i].rect.x, (int)objects[i].rect.y, (int)objects[i].rect.width, (int)objects[i].rect.height, objects[i].prob);
    //     result += temp;
    // }
}
...
std::vector<std::string> getFilenames(std::string dir_name) {
...
int main(int argc, char** argv) {
    ...
    const std::string input_images_path {argv[3]};
